...
class CMakeBuild(build_ext):
    def run(self):
        ...
        #    raise RuntimeError("CMake >= 3.5.1 is required to build ispeech")

        # our CMakeLists builds all the extensions at once
        self.build_extensions()

    def build_extensions(self):
        extdir = os.path.abspath("ispeech")
        ...
        cmake_args = [
            ...
            "-DSP_BUILD_LIBRARIES_ONLY=ON",
            "-DSP_BUILD_FOR_PYTHON=ON",
            "-DSP_LIBRARIES_USE_CUDA=" + use_cuda,
            "-DSP_LIBRARIES_USE_KENLM=" + use_kenlm,
            "-DSP_LIBRARIES_USE_MKL=" + use_mkl,
        ]
        ...
        if platform.system() == "Windows":
            ...
            #     cmake_args += ["-A", "x64"]
            # build_args += ["--", "/m"]
            raise RuntimeError("ispeech doesn't support building on Windows yet")
        else:
...
setup(
    name="ispeech",
    version="0.0.1",
    author="Ben(Zhongtao Han)",
    author_email="hanzhongtao@qiyi.com",
    description="ispeech bindings for python",
    long_description="",
    ext_modules=[
        CMakeExtension("ispeech._common"),
        CMakeExtension("ispeech.source"),
        CMakeExtension("ispeech.transform"),
    ],
