...
def test_fft():
    ...
    plt.figure(1)
    plt.subplot(4,1,1)
    plt.plot(np.arange(N),np.array(signal1))
    ...
    plt.plot(np.arange(spectrum_signal2.size)*fs/N,np.exp(-1j*2*np.pi*50)*abs(spectrum_signal1),'g',label='spectrum_signal1 shift')
    ...
    plt.subplot(4,1,3)
    plt.plot(np.arange(spectrum_signal1.size)*fs/N,np.unwrap(np.angle(spectrum_signal1)),'b',label='phase of spectrum_signal1')

    plt.plot(np.arange(spectrum_signal1.size)*fs/N,np.unwrap(np.angle(np.exp(-1j*2*np.pi*50)*spectrum_signal1)),'r',label='phase of shifted spectrum_signal1')
