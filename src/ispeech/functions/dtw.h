/**
*
*是一个音频基础库，功能包括对音频时域和频域的理解和处理，侧重于频域，
*在基础功能之上，实现了可用于语音的分析和合成的多个模型。
*这些基础功能，旨在为音乐和声音应用的实现提供一个起点和基础参考。
*/
...
private:
  ...
  inline double l1diff(const vector<double> &  v, const vector<double> & w) {
    double ans(0);
    for(uint k = 0; k<v.size();++k)
      ans+=abs(v[k]-w[k]);
    return ans;
