...
namespace vfx_ispeech
{
     ...
     */
    PitchDetection::PitchDetection(std::size_t length)
    {
      N=length;
    }
     ...
     */
  double PitchDetection::Detect(const SampleType x[],uint64_t fs,double &pitch,uint64_t &position)
  {
  ...
  double PitchDetection::Detect2(const SampleType x[],uint64_t fs)
  {
    double pitch;
    uint64_t pos;
    return Detect(x,fs,pitch,pos);
  }
