...
def gensine(samplecount,frequency = 1000,samplerate = 441000,amplitude = 1.0):

    data=np.zeros(samplecount)
    phase = 0;
    normalizedFrequency = frequency/float(samplerate);
    for  i  in range(samplecount):
        data[i] = amplitude*np.sin(
            2.0 * np.pi * normalizedFrequency * i + phase * 2.0 * np.pi
        )
