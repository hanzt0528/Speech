...
def speech_hfc(spectrum,fs):
  ...
  for i in range(0,int(spectrum.size)):
    #Masri//更接近于energy
    #hfc+=spectrum[i]*spectrum[i]*bin2hz*i
    #Jensen//对于信号比较弱的情况，不太有效
    #hfc+=spectrum[i]*bin2hz*i*bin2hz*i
    #Brossier
...
def speech_centroid(mX):
  sum1 = 0.0
  sum2 = 0.0
  for k in range(0,mX.size):
    sum1 += k*abs(mX[k])
    sum2 += abs(mX[k])
  centroid = sum1/sum2
  return centroid
...
def test_sineAnal():
    ...
    Ns = 512
    x = speech_model_sine.sineModelSynth(tfreq,tmag,tphase,Ns,H,fs)
    ...
    #energies = 1- energies/max(energies);
    ...
    for i in range(0,numFrames):
        ...
        centroid = speech_centroid(spectrum)
    ...
    hfcs = hfcs/max(hfcs)
    plt.plot(frmTime,  fluxes ,label = 'spectral flux',lw=1.5,marker = 'x')
    plt.plot(frmTime,  hfcs ,label = 'hfc',lw=1.5,marker = 'x')
    plt.plot(frmTime,  centroids ,label = 'centroid',lw=1.5,marker = 'x')

    
    """
    cost_count = 0
    cost_count = min(energies.size,fluxes.size)
    cost_count = min(cost_count,hfcs.size)
    
    costs = energies[:cost_count]*fluxes[:cost_count]*hfcs[:cost_count]

    plt.plot(H*np.arange(cost_count)/float(fs) ,  costs ,label = 'costs',lw=1.5,marker = 'x')
    """
    plt.legend()
